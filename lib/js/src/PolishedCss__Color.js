...
function getContrast(c1, c2) {
  return Polished__Color.getContrast(toString(c1), toString(c2));
}

function getLuminance(color) {
  return Polished__Color.getLuminance(toString(color));
}

function grayscale(color) {
  return fromString$1(Polished__Color.grayscale(toString(color)));
}

function desaturate(color, amount) {
  return fromString$1(Polished__Color.desaturate(toString(color), amount));
}

function complement(color) {
  return fromString$1(Polished__Color.complement(toString(color)));
}

function adjustHue(color, degree) {
  return fromString$1(Polished__Color.adjustHue(toString(color), degree));
}
